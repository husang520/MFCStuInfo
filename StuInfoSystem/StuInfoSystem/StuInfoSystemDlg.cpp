...
void CStuInfoSystemDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//  DDX_Text(pDX, IDC_EDIT2, m_av);
	DDX_Text(pDX, IDC_EDIT3, m_max);
	DDX_Text(pDX, IDC_EDIT4, m_min);
	//  DDX_Text(pDX, IDC_EDIT5, m_pass);
	DDX_Text(pDX, IDC_EDIT2, m_av);
	DDX_Text(pDX, IDC_EDIT5, m_pass);
...
BEGIN_MESSAGE_MAP(CStuInfoSystemDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON8, &CStuInfoSystemDlg::OnBnClickedButton8)
...
BOOL CStuInfoSystemDlg::OnInitDialog()
{
	...
	m_list.InsertColumn(0, _T("姓名"), LVCFMT_LEFT, 120);	// 插入第2列的列名
	m_list.InsertColumn(1, _T("性别"), LVCFMT_LEFT, 120);	// 插入第3列的列名
	m_list.InsertColumn(2, _T("班级"), LVCFMT_LEFT, 120);	// 插入第4列的列名
	m_list.InsertColumn(3, _T("语文"), LVCFMT_LEFT, 120);	// 插入第5列的列名
	m_list.InsertColumn(4, _T("数学"), LVCFMT_LEFT, 120);	// 插入第6列的列名
	m_list.InsertColumn(5, _T("英语"), LVCFMT_LEFT, 120);	// 插入第7列的列名
...
void CStuInfoSystemDlg::OnBnClickedButton2()
{
	...
	InfoDlg dlg;
	if (dlg.DoModal() == IDOK) {	//用户点击确定之后，才会执行下面的程序
...
void CStuInfoSystemDlg::UpdateList()
{
	...
	for (int i = 0; i < Info.size(); i++) {
		m_list.InsertItem(i, Info[i].m_name.c_str());			// 设置第1列(姓名)
		m_list.SetItemText(i, 1, Info[i].m_sex.c_str());		// 设置第2列(性别)
		str.Format(TEXT("%d"), Info[i].m_class_);				// 设置第3列(班级)
		m_list.SetItemText(i, 2, str);
		str.Format(TEXT("%.1lf"), Info[i].m_score1);			// 设置第5列(语文)
		m_list.SetItemText(i, 3, str);
		str.Format(TEXT("%.1lf"), Info[i].m_score2);			// 设置第6列(数学)
		m_list.SetItemText(i, 4, str);
		str.Format(TEXT("%.1lf"), Info[i].m_score3);			// 设置第7列(英语)
...
void CStuInfoSystemDlg::OnBnClickedButton3()
{
	...
	int nIndex = m_list.GetSelectionMark();	//获取选中行号
	if (nIndex == -1) {
		MessageBox(TEXT("请先选择选择要删除的数据！"), TEXT("提示"));
		return;
	}
	else {
		UINT i;
...
void CStuInfoSystemDlg::OnBnClickedButton4()
{
	...
	int nIndex = m_list.GetSelectionMark();	//获取选中行号
	if (nIndex == -1) {
		MessageBox(TEXT("请先选择选择要修改的数据！"), TEXT("提示"));
		return;
	}
	else {
		InfoDlg dlg;
		...
		if (dlg.DoModal() == IDOK) {
			if (SQL.UpdateInfo(Info[nIndex], StuInfo(dlg.m_name.GetBuffer(), dlg.m_sex.GetBuffer(), dlg.m_class, dlg.m_score1, dlg.m_score2, dlg.m_score3))) {
...
void CStuInfoSystemDlg::OnBnClickedButton5()
{
	...
	if (dlg.DoModal() == IDOK) {
		...
		for (int i = 0; i < result.size(); i++) {
			m_list.InsertItem(i, result[i].m_name.c_str());			// 设置第1列(姓名)
			m_list.SetItemText(i, 1, result[i].m_sex.c_str());		// 设置第2列(性别)
			str.Format(TEXT("%d"), result[i].m_class_);				// 设置第3列(班级)
			m_list.SetItemText(i, 2, str);
			str.Format(TEXT("%.1lf"), result[i].m_score1);			// 设置第5列(语文)
			m_list.SetItemText(i, 3, str);
			str.Format(TEXT("%.1lf"), result[i].m_score2);			// 设置第6列(数学)
			m_list.SetItemText(i, 4, str);
			str.Format(TEXT("%.1lf"), Info[i].m_score3);			// 设置第7列(英语)
			m_list.SetItemText(i, 5, str);
...
void CStuInfoSystemDlg::OnBnClickedButton8()
{
	// TODO: 在此添加控件通知处理程序代码
