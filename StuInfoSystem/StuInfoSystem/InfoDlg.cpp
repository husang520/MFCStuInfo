...
void InfoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT6, m_sex);
	DDX_Text(pDX, IDC_EDIT7, m_class);
	DDX_Text(pDX, IDC_EDIT8, m_score1);
	DDX_Text(pDX, IDC_EDIT9, m_score2);
	DDX_Text(pDX, IDC_EDIT10, m_score3);
}
...
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);	//将Dlg中输入的值更新到绑定的变量里面。子Dlg
	//判断在子对话框中的姓名和性别都不为空
